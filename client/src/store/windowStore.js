...
function loadWindows() {
  try {
    ...
    // Clamp positions to be on-screen (handle resolution changes)
    return parsed.map(w => ({
      ...w,
...
/* ─── Init nextZIndex from persisted data ────────────────────────── */
...
export const useWindowStore = create((set, get) => ({
  ...
  openWindow: (appId) => {
    ...
    if (isProject) {
      ...
      defaults = { width: 680, height: 520, title: prettyTitle, icon: '📄' };
    } else {
      defaults = DEFAULT_WINDOWS[appId] || { width: 600, height: 450, title: appId, icon: '📄' };
    }
  ...
  focusWindow: (id) => {
    set(state => ({ windows: state.windows.map(w => w.id === id ? { ...w, zIndex: ++nextZIndex } : w) }));
    saveWindows(get().windows);
  },
  ...
  /** Legacy: still supported for any external callers */
  ...
  /** Nuke all saved state (e.g. "log out") */
